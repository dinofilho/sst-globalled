...
export default function Employees() {
  ...
  /** carrega tudo */
  useEffect(() => {
    const listCompanies = loadCompanies();
    setCompanies(listCompanies);

    const selId = getSelectedCompanyId();
    setSelectedCompanyId(selId);

    const listEmployees = loadEmployees();
    setEmployees(listEmployees);
  }, []);

  /** salva funcionários sempre que mudar */
  useEffect(() => {
  ...
  /** se não tem empresa selecionada, manda selecionar */
  useEffect(() => {
    if (!selectedCompanyId) return;
    // se a empresa selecionada foi apagada, limpa e manda escolher
    const exists = companies.some((c) => c.id === selectedCompanyId);
    if (companies.length > 0 && !exists) {
      localStorage.removeItem(STORAGE_SELECTED_COMPANY);
  ...
  function clearCompanyEmployees() {
    if (!selectedCompanyId) return;
    if (!confirm("Apagar TODOS os funcionários desta empresa (somente neste dispositivo)?")) return;
    setEmployees((prev) => prev.filter((x) => x.companyId !== selectedCompanyId));
    resetForm();
  }

  /** UI: se não tem empresa selecionada */
    ...
    return (
      <div style={page}>
        <div style={card}>
          ...
          <p style={subtitle}>
            Você precisa selecionar uma empresa primeiro.
          </p>
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 10 }}>
            <Link to="/" style={btn}>Voltar Home</Link>
            <Link to="/companies" style={btn}>Cadastrar Empresas</Link>
    ...
    <div style={page}>
      <div style={card}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "grid", gap: 12, marginTop: 14 }}>
          ...
          <TextArea
            label="Observações"
            placeholder="Ex: treinamentos NR, ASO, restrições, etc."
            value={form.notes}
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginTop: 4 }}>
            ...
            <button type="button" onClick={resetForm} style={btnAction}>
              Limpar
            </button>
            <button type="button" onClick={clearCompanyEmployees} style={btnDanger}>
              Apagar Funcionários desta Empresa
            </button>
          </div>
        </form>
      </div>
